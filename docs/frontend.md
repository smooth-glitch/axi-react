...
A **React 19 + Vite** app in [`web/`](../web). It is both the standalone studio (sidebar, builder, records…) and an **embeddable component library** (`@tstruct/react`) built from the same code. (It used to be React Native Web / Expo; see [vite-migration.md](vite-migration.md).)

## Stack

| Concern | Library |
|---|---|
| Build / dev server | Vite 8 (`npm run dev` on port **8081**) |
...
`configure({ apiUrl, getAuthToken, user, headers })`; defaults: `VITE_API_URL` or `http://localhost:4000`. Every call logs request, response/error and duration. Network failures throw `Cannot reach server at …`; HTTP errors throw an `Error` with `.status` and, for validation failures, `.details` (`[{fieldId, message}]`). Every function takes a struct **id or key**:

`listStructs`, `getStruct`, `createStruct`, `updateStruct`, `listRecords(structRef, {ref})`, `getRecord`, `createRecord(structRef, data, {ref, meta})`, `updateRecord(structRef, recordId, data, {ref, meta})`, `fetchSelectionItems(apiUrl)`.

Headers sent: `Content-Type`, `Authorization: Bearer <token>` (when `getAuthToken` is set), `X-Tstruct-User` (when `user` is set), plus custom `headers`.
...
1. **Server**: add to `FIELD_TYPES` (`routes/structs.js`) and a `case` in `validateRecord` (`lib/validate.js`).
