...
// The map (Leaflet) is only downloaded when someone opens it.
  ...
  return (
    <LocBox $invalid={invalid}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {value?.address ? (
          <Text $variant="small" $color="text" $lines={2} data-testid="location-address">
            {value.address}
          </Text>
        ) : null}
        <Text $variant="caption" $color="textMuted">
          {value ? `${value.lat.toFixed(5)}, ${value.lng.toFixed(5)}` : 'Use your device GPS, or pin a spot on the map'}
        </Text>
        {msg ? (
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
        ...
        <Button size="sm" variant="secondary" title="Pick on map" icon={MapIcon} onPress={() => setMapOpen(true)} testID="pick-on-map" />
      </div>
      {mapOpen ? (
        <Suspense fallback={null}>
          <MapPicker
            ...
            onConfirm={(loc) => {
              log.info('location picked on map', loc);
              onChange(loc);
              setMsg(null);
              setMapOpen(false);
            }}
