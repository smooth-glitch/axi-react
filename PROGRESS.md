...
## Erlang backend port
Because the host project's backend is Erlang/OTP (`axi-chat-backend`, `eredis`, hand-rolled `gen_tcp` HTTP), the API was ported to `server-erlang/` **before** touching the host repo. Same HTTP contract, same Redis keys; `tstruct_http:handle/5` is the single entry point so it can be called from the host's `chat_web:dispatch/6`. OTP 27 built-in `json`, `eredis` only dependency. Verified: full e2e 264/264 against it, 20 EUnit tests (`npm run test:erlang`), curl checks (binary + UTF-8 file round-trip, 413 / 201 at 30 / 20 MB, `API_TOKEN`, CORS, keys). Known difference: `mobile` validation uses a compact rule table instead of libphonenumber. New scripts: `npm run dev:erlang`, `npm run test:erlang`. See [docs/erlang-backend.md](docs/erlang-backend.md), including the step-by-step plan for merging into the host. Express (`server/`) is still present until the team confirms the switch; API changes must be made in both. Nothing has been pushed to the host repo (`smooth-glitch/axi-react`); a local clone sits next to this repo (`../axi-react`) for reference only.

## Frontend without a backend (browser storage)
The app no longer talks to any server. Everything the Express/Redis (and Erlang) backend did now runs in the browser: `web/src/core/localApi.js` is a port of the server's rules (struct definition checks, record validation for every field type incl. phone via `libphonenumber-js`, key uniqueness, `ref`/`meta`, option configs, "applicable to", file size limit, 404/409/413) behind the same REST-shaped paths, persisting through `web/src/core/localDb.js` (IndexedDB stores `structs`, `records`, `options`, `files`, `blobs`; in-memory fallback). `core/api.js` keeps every exported function, return value and error shape, so **no screen or component changed**. `configure()` still accepts `apiUrl` / `getAuthToken` / `headers` (ignored) and gains `storageName` and `maxUploadMb`. `npm run dev` now starts only Vite (no Redis, no API); `server/` and `server-erlang/` are kept as legacy, unused code (root scripts renamed `legacy:*`). e2e adapted (fresh browser profile per run, no Redis cleanup, API checks through `fx` -> `localApi.handle`, new check that nothing calls :4000): **265/265 pass with no backend running**; `npm run build` and `npm run build:lib` succeed. Trade-offs, all in [docs/local-storage.md](docs/local-storage.md): data is per browser profile and per origin (iframe embeds and in-page components of a host use different origins, hence different data), not shared between users or devices, and previous Redis data is not migrated.

## Backend folders removed
`server/` (Express + Redis) and `server-erlang/` (Erlang port; it was never committed) were deleted, together with `docs/backend.md`, `docs/redis.md`, `docs/erlang-backend.md`, the `legacy:*` scripts and the related .gitignore entries. The app is frontend-only. Older sections of this log describe the backend as it was at the time; `server/` remains in git history.

## Downgrade to React 18.3.1 / Vite 8.3.0
react and react-dom pinned to 18.3.1, vite to 8.3.0 (exact versions in web/package.json). No code changes were needed (no React 19-only APIs in use). Peer dependencies of framer-motion, react-router-dom, lucide-react and styled-components resolve cleanly; e2e 265/265, `npm run build` and `npm run build:lib` pass.

## Map location picker
The `location` field gained **Pick on map** next to the existing **Capture**: a Blinkit-style dialog (`web/src/ui/MapPicker.jsx`) with a fixed centre pin over a draggable Leaflet / OpenStreetMap map, address search, **Use my current location**, live address of the pin (Nominatim), Confirm / Cancel / Esc. Stored value stays `{ lat, lng }` (validation unchanged) plus an optional `address`. No API key; Leaflet 1.9.4 (exact) is lazy-loaded, its CSS injected by the component, and the library build stays one file (`inlineDynamicImports`, about +60 kB gzipped). Needs internet for tiles / address lookup (degrades gracefully); see the privacy and CSP notes in docs/field-types-and-conditions.md. e2e: 281 checks (new `map picker` step with mocked tiles and Nominatim).
