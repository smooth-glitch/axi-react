// End-to-end test: drives the real UI in Chrome (Playwright). Needs only the web app (:8081) - there is no backend:
// all data lives in the browser (IndexedDB), and every run starts in a brand-new browser profile, so nothing needs cleaning up.
// Run: cd e2e && npm i && node e2e.js   (CHROME_PATH / APP_URL / SHOTS_DIR env vars override defaults)
// The API-contract checks talk to the app's in-browser store (src/core/localApi.js) through `fx`, a fetch()-shaped helper.
const { chromium } = require('playwright-core');
const http = require('http');
const APP = process.env.APP_URL || 'http://localhost:8081', API = ''; // API is only the (empty) prefix of the REST-shaped paths
...
(async () => {
  ...
  const backendCalls = []; // any request to the old API port would mean something is still wired to a backend
  p.on('request', r => { if (/^https?:\/\/(localhost|127\.0\.0\.1):4000\//.test(r.url())) backendCalls.push(r.url()); });
  ...
  // fetch()-shaped access to the in-browser store: fx('/api/structs', { method, headers, body }) -> { status, ok, json(), arrayBuffer() }
  const fx = async (url, init = {}) => {
    const method = (init.method || 'GET').toUpperCase();
    const headers = Object.fromEntries(Object.entries(init.headers || {}).map(([k, v]) => [k.toLowerCase(), v]));
    let body = null, file = null;
    if (init.body && typeof init.body !== 'string') { // multipart upload: FormData with a "file" part
      const f = init.body.get('file');
      if (f) file = { name: f.name, type: f.type, bytes: Array.from(new Uint8Array(await f.arrayBuffer())) };
    } else if (init.body) body = JSON.parse(init.body);
    const out = await p.evaluate(async ({ method, path, body, file, user }) => {
      const { handle } = await import('/src/core/localApi.js');
      if (file) body = { file: new File([new Uint8Array(file.bytes)], file.name, { type: file.type }) };
      const r = await handle(method, path, body, { user });
      const blob = r.body && r.body.blob;
      const bytes = blob ? Array.from(new Uint8Array(await blob.arrayBuffer())) : null;
      return { status: r.status, body: blob ? { ...r.body, blob: undefined } : r.body, bytes };
    }, { method, path: url, body, file, user: headers['x-tstruct-user'] || null });
    return { status: out.status, ok: out.status < 400, json: async () => out.body, arrayBuffer: async () => Uint8Array.from(out.bytes || []).buffer };
  };
  ...
  await step('shell', async () => {
    // the store of a fresh profile is empty: add one small struct + record so the overview shows its normal (non-empty) content
    const seed = await (await fx('/api/structs', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'Seed struct', fields: [{ id: 'a', label: 'A', type: 'text' }] }) })).json();
    await fx(`/api/structs/${seed.structId}/records`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ data: { a: 'x' } }) });
    await p.goto(APP); await p.waitForTimeout(2500);
  ...
  await step('stored def', async () => {
    const def = (await (await fx(`${API}/api/structs/${structId}`)).json()).struct;
  ...
  await step('records', async () => {
    ...
    const recs = (await (await fx(`${API}/api/structs/${structId}/records`)).json()).records;
    const d = recs[0].data;
  ...
  await step('edit record', async () => {
    const list = async () => (await (await fx(`${API}/api/structs/${structId}/records`)).json()).records;
  ...
  await step('dropdown', async () => {
    const post = (path, body) => fx(API + path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json());
  ...
  // ---------------------------------------------------------------- map location picker
  await step('map picker', async () => {
    // OpenStreetMap tiles + Nominatim are mocked: fast, offline and deterministic
    const PNG = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==', 'base64');
    let searches = 0;
    await p.route('https://tile.openstreetmap.org/**', r => r.fulfill({ status: 200, contentType: 'image/png', body: PNG }));
    await p.route('https://nominatim.openstreetmap.org/**', r => {
      const u = new URL(r.request().url());
      const cors = { 'access-control-allow-origin': '*' };
      if (u.pathname === '/reverse') return r.fulfill({ status: 200, contentType: 'application/json', headers: cors, body: JSON.stringify({ display_name: `Pin at ${Number(u.searchParams.get('lat')).toFixed(3)}, Test Street, Bengaluru` }) });
      searches++;
      return r.fulfill({ status: 200, contentType: 'application/json', headers: cors, body: JSON.stringify([{ place_id: 1, lat: '13.0100', lon: '77.6400', display_name: 'Indiranagar, Bengaluru, Karnataka' }]) });
    });
    const mk = await (await fx('/api/structs', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'MapTest', fields: [{ id: 'loc', label: 'Delivery spot', type: 'location', required: true }] }) })).json();
    await p.goto(`${APP}/structs/${mk.structId}/form`); await p.waitForTimeout(1500);
    check('map: field offers Capture and Pick on map', (await has('Capture')) && (await tid('pick-on-map').count()) === 1);

    // open, use the (mocked) current location, confirm
    await tid('pick-on-map').click(); await p.waitForTimeout(700);
    check('map: picker dialog opens with a Leaflet map', (await tid('map-picker').isVisible()) && (await p.locator('.leaflet-container').count()) === 1);
    check('map: search box + current-location button present', (await tid('map-search').count()) === 1 && (await tid('map-use-current').isVisible()));
    await tid('map-use-current').click(); await p.waitForTimeout(2500);
    let addr = await tid('map-address').innerText();
    check('map: current location moves the pin and looks up the address', /Test Street/.test(addr) && /12\.9/.test(addr), addr);
    check('map: coordinates shown under the address', (await tid('map-picker').innerText()).includes('12.97'), (await tid('map-picker').innerText()).slice(-160));
    await p.screenshot({ path: `${SHOTS}/map_picker.png` });
    await tid('map-confirm').click(); await p.waitForTimeout(600);
    check('map: dialog closes after Confirm', (await tid('map-picker').count()) === 0);
    check('map: field shows Location captured + the address', (await has('Location captured')) && /Test Street/.test(await tid('location-address').innerText()));

    // search for a place, pick the result, confirm
    await tid('pick-on-map').click(); await p.waitForTimeout(700);
    check('map: reopening starts at the stored spot (address kept)', /Test Street/.test(await tid('map-address').innerText()));
    await tid('map-search').fill('Indiranagar'); await p.waitForTimeout(1500);
    check('map: search lists results', (await has('Indiranagar, Bengaluru, Karnataka')) && searches >= 1);
    await p.getByText('Indiranagar, Bengaluru, Karnataka').first().click(); await p.waitForTimeout(2500);
    check('map: choosing a result moves the pin there', (await tid('map-picker').innerText()).includes('13.01'), (await tid('map-picker').innerText()).slice(-160));
    await tid('map-confirm').click(); await p.waitForTimeout(600);

    // cancel and Escape leave the value alone
    await tid('pick-on-map').click(); await p.waitForTimeout(600);
    await tid('map-cancel').click(); await p.waitForTimeout(500);
    check('map: X closes without changing the value', (await tid('map-picker').count()) === 0 && (await has('13.01')));
    await tid('pick-on-map').click(); await p.waitForTimeout(600);
    await p.keyboard.press('Escape'); await p.waitForTimeout(500);
    check('map: Escape closes the picker', (await tid('map-picker').count()) === 0);

    // submit and check what was stored
    await tid('submit').click(); await p.waitForTimeout(1200);
    const saved = (await (await fx(`/api/structs/${mk.structId}/records`)).json()).records[0];
    check('map: stored value has lat/lng of the chosen place', Math.abs(saved?.data.loc?.lat - 13.01) < 0.01 && Math.abs(saved?.data.loc?.lng - 77.64) < 0.01, JSON.stringify(saved?.data));
    check('map: stored value keeps the address', /Test Street/.test(saved?.data.loc?.address || ''), JSON.stringify(saved?.data));

    // the old quick "Capture" button still works and drops the address
    await p.goto(`${APP}/structs/${mk.structId}/record/${saved.id}`); await p.waitForTimeout(1500);
    check('map: edit mode pre-fills the stored location and address', (await has('13.01000, 77.64000')) && (await has('Test Street')));
    await click('Recapture'); await p.waitForTimeout(1500);
    check('map: quick Recapture still works (GPS coordinates, no address)', (await has('12.97160, 77.59460')) && (await tid('location-address').count()) === 0);
    await p.unroute('https://tile.openstreetmap.org/**'); await p.unroute('https://nominatim.openstreetmap.org/**');
  });
  ...
  await step('narrow layout', async () => {
    const q = await ctx.newPage(); // same browser profile (so it sees the data created above), phone-sized viewport
    await q.setViewportSize({ width: 400, height: 860 });
  ...
  await step('options', async () => {
    ...
    const post = (path, body) => fx(API + path, { method: 'POST', headers: H, body: JSON.stringify(body) });
    ...
    const getOpt = async (id) => (await (await fx(`${API}/api/options/${id}`)).json()).option;
    const findOpt = async (caption) => (await (await fx(`${API}/api/options`)).json()).options.find(o => o.caption === caption);
    ...
    check('options RUN dataInput: record saved through that form', ((await (await fx(`${API}/api/structs/${target.structId}/records`)).json()).records[0]?.data.note) === 'from an option');
    // struct not found
    ...
    const meta = (await (await fx(`${API}/api/files`)).json()).files.find(f => f.id === dl1.config.fileId);
    ...
    check('options RUN upload: fileId is a real stored file', /^[0-9a-f-]{36}$/.test(newId) && (await fx(`${API}/api/files/${newId}`)).status === 200);
    const apiBytes = Buffer.from(await (await fx(`${API}/api/files/${newId}`)).arrayBuffer());
    ...
    check('options delete: gone from the API (404)', (await fx(`${API}/api/options/${di.id}`)).status === 404);
    check('options delete: an attached file survives deleting an option', (await fx(`${API}/api/files/${dl1.config.fileId}`)).status === 200);
  ...
  await step('embedding', async () => {
    ...
    let r = await fx(`${API}/api/structs/e2e-leave`); const byKey = (await r.json()).struct;
    check('embed: GET struct by key', r.status === 200 && byKey.id === structId);
    r = await fx(`${API}/api/structs`, { method: 'POST', headers: H, body: JSON.stringify({ name: 'Dup', key: 'e2e-leave', fields: [{ id: 'a', label: 'A', type: 'text' }] }) });
    check('embed: duplicate key -> 409', r.status === 409);
    r = await fx(`${API}/api/structs`, { method: 'POST', headers: H, body: JSON.stringify({ name: 'Bad', key: '9x', fields: [{ id: 'a', label: 'A', type: 'text' }] }) });
    check('embed: invalid key -> 400', r.status === 400);
    const listed = (await (await fx(`${API}/api/structs`)).json()).structs.find(s => s.id === structId);
    check('embed: list exposes key', listed.key === 'e2e-leave');
    const good = { employmentType: 'Contract', days: 4, startDate: '2026-12-01' };
    r = await fx(`${API}/api/structs/e2e-leave/records`, { method: 'POST', headers: { ...H, 'X-Tstruct-User': 'crm-bot' }, body: JSON.stringify({ data: good, ref: 'order-7', meta: { src: 'api' } }) });
    ...
    let refd = (await (await fx(`${API}/api/structs/e2e-leave/records?ref=order-7`)).json()).records;
    check('embed: filter records by ref', refd.length === 1 && refd[0].id === made.id);
    r = await fx(`${API}/api/structs/e2e-leave/records`, { method: 'POST', headers: H, body: JSON.stringify({ data: good, ref: 123 }) });
    check('embed: non-string ref -> 400', r.status === 400);
    r = await fx(`${API}/api/structs/e2e-leave/records/${made.id}`, { method: 'PUT', headers: H, body: JSON.stringify({ data: good, ref: null }) });
    ...
    const hostRec = (await (await fx(`${API}/api/structs/e2e-leave/records?ref=order-42`)).json()).records;
    ...
    check('host component: edit saved + event', /submitted edit id=/.test(await tid('host-events').innerText()) && (await (await fx(`${API}/api/structs/e2e-leave/records/${hostRec[0].id}`)).json()).record.data.days === 9);
    ...
    const ifr = (await (await fx(`${API}/api/structs/e2e-leave/records?ref=order-42`)).json()).records;
  ...
  await step('server', async () => {
    const H = { 'content-type': 'application/json' };
    const post = (path, body) => fx(API + path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }).then(async r => [r.status, await r.json()]);
    const put = (path, body) => fx(API + path, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }).then(async r => [r.status, await r.json()]);
    ...
    const r = await fx(`${API}/api/structs/${structId}/records/nope`); check('api: missing record -> 404', r.status === 404);
    const someRec = (await (await fx(`${API}/api/structs/${structId}/records`)).json()).records[0];
    ...
    const list = (await (await fx(`${API}/api/structs`)).json()).structs.find(x => x.id === structId);
    check('api: list has field/section/record counts + modifiedAt', list.fieldCount === 15 && list.sectionCount === 1 && list.recordCount === (await (await fx(`${API}/api/structs/${structId}/records`)).json()).records.length && !!list.modifiedAt, JSON.stringify(list));
    // files + options API
    const fj = (path, init) => fx(API + path, init).then(async r => [r.status, await r.json().catch(() => ({}))]);
  ...
  const noise = errs.filter(e => !/404 \(Not Found\)|xx (GET|PUT|POST|DELETE) \/api\/|xx GET .*does-not-exist|xx \(selection\) http:\/\/localhost:4199|ERR_CONNECTION_REFUSED|Failed to load resource|\[warn\]/.test(e));
  ...
  check('no requests to a backend (:4000): everything runs in the browser', backendCalls.length === 0, backendCalls.slice(0, 3).join(' | '));
