...
* **Backend**: untouched by the migration (it has since been removed: data now lives in the browser, see [local-storage.md](local-storage.md)).
...
| `EXPO_PUBLIC_API_URL` | `VITE_API_URL` (later removed together with the backend, see [local-storage.md](local-storage.md)) |
